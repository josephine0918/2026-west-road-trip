...
function renderLife(){ const rows=activeLife==='costco'?APP.costcos:APP.stays; $('#lifeContent').innerHTML=`<div class="life-grid">${rows.map(r=>{ const q=r[3]||r[2]; return `<article class="life-card"><small>${r[0]}</small><h3>${r[1]}</h3><p>${r[2]}</p><div class="card-actions"><a class="pill-btn" target="_blank" rel="noopener" href="${nav(q)}">🧭 ${C('nav')}</a><a class="map-btn" target="_blank" rel="noopener" href="${maps(q)}">📍 ${C('map')}</a></div></article>`}).join('')}</div>`; $$('.segmented button').forEach(b=>b.classList.toggle('active',b.dataset.life===activeLife)); }
